-- ─── PROCEDURE→FUNCTION: note_getallnoteofuser_total ───────────────────────────────
...
DROP FUNCTION IF EXISTS public.note_getallnoteofuser_total(integer, character varying, character varying, integer, double precision, double precision, integer);
CREATE OR REPLACE FUNCTION public.note_getallnoteofuser_total(
    IN userno integer,
    IN groupno character varying,
    IN textsearch character varying,
    IN type integer,
    IN lng1 double precision,
    IN lat1 double precision DEFAULT 0,
...
BEGIN
	...
	SELECT DepartNo INTO departno FROM Organization_BelongToDepartment WHERE UserNo = note_getallnoteofuser_total.userno;
	...
	END LOOP;
		...
		WHERE (CASE Type WHEN 0 THEN N.UserNo 
				...
				END = note_getallnoteofuser_total.userno 
				OR CASE Type WHEN 0 THEN N.ListNo 
							WHEN 2 THEN N.ListNo 
					END in (select S.ListNo from Note_Share S where (S.ShareType = 2 AND S.UserShare = note_getallnoteofuser_total.userno AND S.UserNo <> note_getallnoteofuser_total.userno))) -- OR (S.ShareType = 0 AND S.UserShare IN(SELECT * FROM DepartNos))))
			...
			AND (FavoriteType=-1 OR (N.FavoriteType = note_getallnoteofuser_total.favoritetype AND UserNo =  N.UserNo)  OR (UserNo <>  N.UserNo AND N.ListNo IN (SELECT ListNo FROM Note_Share WHERE  FavoriteType = note_getallnoteofuser_total.favoritetype AND UserShare = note_getallnoteofuser_total.userno 
									AND ListNo = N.ListNo)));
