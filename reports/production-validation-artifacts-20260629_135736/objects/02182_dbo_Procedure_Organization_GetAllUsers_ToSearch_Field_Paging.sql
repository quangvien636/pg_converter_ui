-- ─── PROCEDURE→FUNCTION: organization_getallusers_tosearch_field_paging ───────────────────────────────
-- NOTE: SQL Server stored procedure converted to PostgreSQL function.
-- TODO: Review converted output — stored procedure semantics differ; test before use in production.
-- TODO: replace SETOF record — procedure returns results; add RETURNS TABLE(col type, ...) manually
-- TODO: procedure contains result-returning SELECT; replace SETOF record with correct column types
DROP FUNCTION IF EXISTS public.organization_getallusers_tosearch_field_paging(integer, integer, boolean, character varying);
CREATE OR REPLACE FUNCTION public.organization_getallusers_tosearch_field_paging(
...
BEGIN
	...
	IF AlsoDisabled = 0 THEN
		...
		(CASE TreeType
			...
			WHEN 'userid' THEN U.Userid
			WHEN 'mailaddress' THEN U.MailAddress
			WHEN 'companyphone' THEN U.CompanyPhone
			WHEN 'extensionnumber' THEN U.ExtensionNumber
			WHEN 'cellphone' THEN U.CellPhone
			WHEN 'departname' THEN D.Name
		END) ILIKE '%' || SearchText || '%'
	...
	INNER JOIN  Organization_BelongToDepartment B ON U.UserNo = B.UserNo
