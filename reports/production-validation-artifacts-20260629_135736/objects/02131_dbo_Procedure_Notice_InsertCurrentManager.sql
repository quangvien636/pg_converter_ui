-- ─── PROCEDURE→FUNCTION: notice_insertcurrentmanager ───────────────────────────────
...
IF (SELECT COUNT(*) FROM Notice_UserByGroup WHERE USER_NO=notice_insertcurrentmanager.user_no AND DEPARTMENT_ID=notice_insertcurrentmanager.department_id) <=0 THEN;
