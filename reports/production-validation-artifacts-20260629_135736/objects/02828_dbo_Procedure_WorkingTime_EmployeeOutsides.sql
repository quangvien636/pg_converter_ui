-- ─── PROCEDURE→FUNCTION: workingtime_employeeoutsides ───────────────────────────────
-- NOTE: SQL Server stored procedure converted to PostgreSQL function.
-- TODO: Review converted output — stored procedure semantics differ; test before use in production.
DROP FUNCTION IF EXISTS public.workingtime_employeeoutsides(integer, integer, integer, integer, character varying, character varying, character varying);
CREATE OR REPLACE FUNCTION public.workingtime_employeeoutsides(
    IN p_from integer,
    IN p_to integer,
...
BEGIN
	...
	LEFT JOIN Organization_SortingEachDepartment S ON S.UserNo = U.UserNo and S.DepartNo = workingtime_employeeoutsides.p_departno
	...
	AND ( (p_GroupNo = 0 OR U.GroupId = workingtime_employeeoutsides.p_groupno) OR (p_GroupNo !=  0 AND U.GroupId = workingtime_employeeoutsides.p_groupno))
	AND ( (p_GroupNo = 0 OR G.ID = workingtime_employeeoutsides.p_groupno) OR (p_GroupNo !=  0 AND G.ID = workingtime_employeeoutsides.p_groupno))
