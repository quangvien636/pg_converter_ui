-- ─── PROCEDURE→FUNCTION: notice_mobile_getlistofnotices ───────────────────────────────
...
DROP FUNCTION IF EXISTS public.notice_mobile_getlistofnotices(integer, character varying, integer, boolean, integer, integer);
CREATE OR REPLACE FUNCTION public.notice_mobile_getlistofnotices(
    IN userno integer,
...
BEGIN
	...
	Query := 'SELECT TOP (' || CONVERT(NVARCHAR, CountOfArticles) + ') N.NoticeNo, N.RegUserNo, N.RegDate, N.Title, ' +;
			...
			'(SELECT COUNT(*) FROM NoticeReference R WHERE R.NoticeNo = N.NoticeNo) AS CountRead, ' +
			'(SELECT COUNT(*) FROM NoticeComments C WHERE C.NoticeNo = N.NoticeNo) AS CountComment ' +
		'FROM Notices N ' +
		'INNER JOIN NoticeDivisions D ON D.DivisionNo = N.DivisionNo '
