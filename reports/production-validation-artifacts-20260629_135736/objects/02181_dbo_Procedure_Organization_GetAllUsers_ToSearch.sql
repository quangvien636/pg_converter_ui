-- ─── PROCEDURE→FUNCTION: organization_getallusers_tosearch ───────────────────────────────
-- NOTE: SQL Server stored procedure converted to PostgreSQL function.
-- TODO: Review converted output — stored procedure semantics differ; test before use in production.
-- TODO: replace SETOF record — procedure returns results; add RETURNS TABLE(col type, ...) manually
-- TODO: procedure contains result-returning SELECT; replace SETOF record with correct column types
DROP FUNCTION IF EXISTS public.organization_getallusers_tosearch(boolean);
CREATE OR REPLACE FUNCTION public.organization_getallusers_tosearch(
...
BEGIN
	...
	IF AlsoDisabled = 0 THEN
		...
		WHERE B.IsDefault = TRUE
			AND (U.UserID ILIKE '%' || SearchText || '%' OR U.Name ILIKE '%' || SearchText || '%' OR U.Name_EN ILIKE '%' || SearchText || '%')
	...
	INSERT INTO ListOfUsers
	RETURN QUERY
	SELECT U.UserNo, U.ModUserNo, U.ModDate, U.UserID, U.Name, U.Name_EN, U.MailAddress, U.CellPhone, U.CompanyPhone, U.UserPhoto, U.Photo, U.Enabled,
		0, '', '', 9999,
		0, '', '', 9999,
		0, '', '', 9999,
		U.BirthDate
