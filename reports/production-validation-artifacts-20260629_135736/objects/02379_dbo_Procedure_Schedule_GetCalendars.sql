-- ─── PROCEDURE→FUNCTION: schedule_getcalendars ───────────────────────────────
...
BEGIN
	...
	END;
	IF (SELECT COUNT(UserNo) FROM ScheduleCalendarSetup WHERE UserNo = schedule_getcalendars.userno) > 0 THEN;
	...
	ELSE
		...
		CalNo := (select /* TOP 1 */ CalendarNo FROM ScheduleCalendars WHERE RegUserNo = schedule_getcalendars.userno AND IsFixed = TRUE) --2023;;
