-- ─── PROCEDURE→FUNCTION: mail_insertnewaccount ───────────────────────────────
...
BEGIN
	...
	SELECT Value INTO mailsize from Center_Configuration WHERE key = 'KEY_USE_USER_MAILSIZE'
	IF(MailSize is null)
	BEGIN
	 MailSize := 20480;
	END;
	...
	SELECT Password, Name INTO password, name FROM Organization_Users
	WHERE UserNo = mail_insertnewaccount.userno

	SELECT  INTO  FROM Mail_Servers
