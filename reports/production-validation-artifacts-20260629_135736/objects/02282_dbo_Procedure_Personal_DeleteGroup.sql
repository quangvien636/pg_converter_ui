-- ─── PROCEDURE→FUNCTION: personal_deletegroup ───────────────────────────────
-- NOTE: SQL Server stored procedure converted to PostgreSQL function.
-- TODO: Review converted output — stored procedure semantics differ; test before use in production.
-- TODO: LEN was not fully converted; use length()
DROP FUNCTION IF EXISTS public.personal_deletegroup();
CREATE OR REPLACE FUNCTION public.personal_deletegroup(
) RETURNS void
AS $function$
DECLARE
    groupno integer;
-- !! WARNING: output needs manual review — see TODO comments
BEGIN

	GroupNoList := GroupNoList || ',';
	WHILE STRPOS(',GroupNoList, ') > 0 LOOP
		GroupNo := SUBSTRING(GroupNoList,0,STRPOS(',GroupNoList, '));;
		DELETE FROM PersonalGroupUser WHERE GroupNo = GroupNo;
		DELETE FROM PersonalGroup WHERE GroupNo = GroupNo
		
		GroupNoList := SUBSTRING(GroupNoList,STRPOS(',GroupNoList, ')+1,LEN(GroupNoList));
	END LOOP;
END;

/****** Object:  StoredProcedure public."Personal_CheckPassword"    Script Date: 11/13/2014 20:06:56 ******/
END;
$function$
LANGUAGE plpgsql;
-- TODO: Owner mapping skipped. Target role postgres not verified.
