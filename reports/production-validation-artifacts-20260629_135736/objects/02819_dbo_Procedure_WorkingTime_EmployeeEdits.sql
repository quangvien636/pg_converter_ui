-- ─── PROCEDURE→FUNCTION: workingtime_employeeedits ───────────────────────────────
-- NOTE: SQL Server stored procedure converted to PostgreSQL function.
-- TODO: Review converted output — stored procedure semantics differ; test before use in production.
-- TODO: replace SETOF record — procedure returns results; add RETURNS TABLE(col type, ...) manually
-- TODO: procedure contains result-returning SELECT; replace SETOF record with correct column types
DROP FUNCTION IF EXISTS public.workingtime_employeeedits(integer, integer, integer, integer, character varying, character varying, character varying);
CREATE OR REPLACE FUNCTION public.workingtime_employeeedits(
    IN p_from integer,
    IN p_to integer,
...
BEGIN
	...
	LEFT JOIN Organization_SortingEachDepartment S ON S.UserNo = U.UserNo and S.DepartNo = workingtime_employeeedits.p_departno
	...
	AND ( (p_GroupNo = 0 OR U.GroupId = workingtime_employeeedits.p_groupno) OR (p_GroupNo !=  0 AND U.GroupId = workingtime_employeeedits.p_groupno))
	AND ( (p_GroupNo = 0 OR G.ID = workingtime_employeeedits.p_groupno) OR (p_GroupNo !=  0 AND G.ID = workingtime_employeeedits.p_groupno))
