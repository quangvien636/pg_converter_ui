-- ─── PROCEDURE→FUNCTION: organization_getusersbydepartment_paging ───────────────────────────────
-- NOTE: SQL Server stored procedure converted to PostgreSQL function.
-- TODO: Review converted output — stored procedure semantics differ; test before use in production.
-- TODO: replace SETOF record — procedure returns results; add RETURNS TABLE(col type, ...) manually
-- TODO: procedure contains result-returning SELECT; replace SETOF record with correct column types
DROP FUNCTION IF EXISTS public.organization_getusersbydepartment_paging(integer, integer, integer, boolean);
CREATE OR REPLACE FUNCTION public.organization_getusersbydepartment_paging(
...
BEGIN
	...
	INSERT INTO Sorting
	RETURN QUERY
	SELECT UserNo FROM Organization_SortingEachDepartment WHERE DepartNo = organization_getusersbydepartment_paging.departno
	...
	IF AlsoDisabled = 0 THEN
		...
		WHERE 
			B.DepartNo = organization_getusersbydepartment_paging.departno
		ORDER BY 
	...
	IF (SELECT COUNT(*) FROM Sorting) = 0 THEN
		...
		(
			SELECT 
				ROW_NUMBER() OVER(ORDER BY PositionSortNo,Name) AS rowNum, 
				RowIdx,
				UserNo, 
				ModUserNo, 
				ModDate, 
	...
	ELSE BEGIN
		...
		RETURN QUERY
		SELECT S.RowIdx as RowIdx, L.UserNo, L.ModUserNo, L.ModDate, L.UserID, L.Name, L.Name_EN, L.MailAddress, L.CellPhone, L.CompanyPhone,L.ExtensionNumber, L.UserPhoto, L.Photo,
		...
		UPDATE ListOfUsers2 SET RowIdx = (SELECT MAX(COALESCE(RowIdx,9999)) FROM ListOfUsers2 A WHERE A.PositionSortNo >= PositionSortNo)
		...
		(
			SELECT 
				ROW_NUMBER() OVER(ORDER BY RowIdx,PositionSortNo,Name) AS rowNum,
				RowIdx,
				UserNo,
				ModUserNo, 
				ModDate, 
