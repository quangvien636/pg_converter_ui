-- ─── PROCEDURE→FUNCTION: organization_getusersofdepartmentandincludechild_paging ───────────────────────────────
-- NOTE: SQL Server stored procedure converted to PostgreSQL function.
-- TODO: Review converted output — stored procedure semantics differ; test before use in production.
-- TODO: replace SETOF record — procedure returns results; add RETURNS TABLE(col type, ...) manually
-- TODO: procedure contains result-returning SELECT; replace SETOF record with correct column types
DROP FUNCTION IF EXISTS public.organization_getusersofdepartmentandincludechild_paging(integer, integer, integer, boolean);
CREATE OR REPLACE FUNCTION public.organization_getusersofdepartmentandincludechild_paging(
...
BEGIN
	...
	INSERT INTO Sorting
	RETURN QUERY
	SELECT UserNo FROM Organization_SortingEachDepartment WHERE DepartNo = organization_getusersofdepartmentandincludechild_paging.departno
	...
	IF AlsoDisabled = 0 THEN
		...
		WHERE 
			B.DepartNo IN (SELECT DepartNo FROM public."Organization_GetDepartments_Reflexive"(DepartNo, AlsoDisabled))
