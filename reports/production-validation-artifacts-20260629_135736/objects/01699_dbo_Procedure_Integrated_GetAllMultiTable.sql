-- ─── PROCEDURE→FUNCTION: integrated_getallmultitable ───────────────────────────────
...
BEGIN
	...
	SELECT WorkPlaceType INTO checkhq from Organization_Users where UserNo=integrated_getallmultitable.userno
...
if(BoardNo>-1) BEGIN
	Query := 'SELECT T.TotalCnt,T.RowNum,T.Id ,T.Title,T.RegUserNo, T.RegDate, T.PositionName, T.DepartName , T.CatName, T.UserName, T.ReadCount, 0 as TableType';
	SET Query += ' FROM ('
...
sql := Query;
	Query3 := Query || ' ORDER BY RegDate ' || OrderBy;
END;
	/*
	 * 게시글 검색 시작
	 */
	...
	SELECT /* TOP 1 */ DepartNo = DepartNo FROM Organization_BelongToDepartment WHERE UserNo = integrated_getallmultitable.userno
		 ...
		 if(TreeRoot>-1) begin
	Query1 := 'SELECT T.TotalCnt,T.RowNum,CONVERT(int,T.IntegratedNo) as Id ,T.Title,T.RegUserNo, T.RegDate,  T.PositionName, T.Department as DepartName, T.TreeName as CatName, T.UserName, T.ViewUserCnt as ReadCount';
	...
	sql := Query1;
	Query3 := Query1 || ' ORDER BY RegDate ' || OrderBy;
	END;
	
	---------------------- NoticeSyn --------------------------
	if(DivisionNo>-1) begin
	Query2 := 'SELECT T.TotalCnt,T.RowNum,CONVERT(int, T.NoticeNo) as Id ,T.Title,T.RegUserNo, T.RegDate,T.PositionName, T.Department as DepartName, T.DivisionName as CatName, T.UserName, T.ViewUserCnt as ReadCount';
	...
	SET Query2 +=  'WHERE T.RowNum BETWEEN ' || CONVERT(NVARCHAR(20), ((CurrentPageIndex - 1) * CountPerPage) + 1) + ' AND ' || CONVERT(NVARCHAR(20), CurrentPageIndex * CountPerPage) + ' '
	 ...
	 sql := Query2;
	Query3 := Query2 || ' ORDER BY T.Important DESC, T.RegDate DESC ' || OrderBy;
	 END;


	 PERFORM query3();
	 count := ' SELECT count(*) as TotalCnt FROM (' || sql || ') as C';
	 RAISE NOTICE '%', Query3;
	PERFORM count();
END;
$function$
