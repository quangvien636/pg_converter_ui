-- ─── PROCEDURE→FUNCTION: mail_getmails_shared ───────────────────────────────
...
DROP FUNCTION IF EXISTS public.mail_getmails_shared(integer, integer, character varying, bigint, bigint, bigint, integer, bigint, integer, boolean, integer, integer, boolean);
CREATE OR REPLACE FUNCTION public.mail_getmails_shared(
    IN userno integer,
...
BEGIN
	...
	ELSIF (SortColumn = 9) SET Query += 'COALESCE((select /* TOP 1 */ CommentsType from Mail_SharedComments where MailNo = Mail_Mails.MailNo  order by ModDate desc),0) ' THEN
	ELSIF (SortColumn = 10) SET Query += '(select count(*) from Mail_SharedReference where MailNo = Mail_Mails.MailNo) ' THEN

/*
	,(select count(*) from Mail_SharedReference where MailNo = M.MailNo) as SharedReferenceCount
	,COALESCE((select /* TOP 1 */ CommentsType from Mail_SharedComments where MailNo = M.MailNo  order by ModDate desc),0) as CommentsType
*/
	...
	EndRowNum := mail_getmails_shared.currentpageindex * CountPerPage;
	IF ParentNo = -1 AND SortNo = 1 THEN
		...
		LEFT JOIN (SELECT TagNo, ImageNo FROM Mail_MailTags WHERE UserNo = mail_getmails_shared.userno) MT ON MT.TagNo = M.TagNo
	...
	IF IsConversation = TRUE THEN
		...
		WHERE UserNo = mail_getmails_shared.userno AND IsDelete = FALSE AND ConversationNo IN (SELECT T3.ConversationNo FROM SearchResult3 T3)
