-- ─── PROCEDURE→FUNCTION: workingtime_reports2 ───────────────────────────────
...
BEGIN
	...
	WHILE @FETCH_STATUS = 0 BEGIN
		...
		TotalLate := 0;
		TotalEdit := 0;
		SELECT sum(c.TimeWork) INTO totaltime from WorkingTime_Calculater c where c.UserNo = _UserNo and  c.status = 1 and c.WorkingDay BETWEEN FromDate AND ToDate
		SELECT (TotalDays  - COUNT(*)) INTO totaldayoff from WorkingTime_Calculater c where c.UserNo = _UserNo  and c.WorkingDay BETWEEN FromDate AND ToDate
