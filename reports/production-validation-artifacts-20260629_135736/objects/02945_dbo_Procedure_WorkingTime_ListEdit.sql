-- ─── PROCEDURE→FUNCTION: workingtime_listedit ───────────────────────────────
-- NOTE: SQL Server stored procedure converted to PostgreSQL function.
-- TODO: Review converted output — stored procedure semantics differ; test before use in production.
DROP FUNCTION IF EXISTS public.workingtime_listedit(timestamp without time zone, timestamp without time zone, integer, integer, character varying, character varying, character varying);
CREATE OR REPLACE FUNCTION public.workingtime_listedit(
...
BEGIN
	...
	LEFT JOIN Organization_SortingEachDepartment S ON S.UserNo = U.UserNo and S.DepartNo = workingtime_listedit.p_departno
	...
	AND ( (p_GroupNo = 0 OR U.GroupId = workingtime_listedit.p_groupno) OR (p_GroupNo !=  0 AND U.GroupId = workingtime_listedit.p_groupno))
	...
	FROM ListOfUsers L
	JOIN (
					SELECT t.UserNo, t.CheckTimeFull
					FROM WorkingTime_Times t
					where t.Provider = 999 and t.CheckTimeFull BETWEEN p_From AND p_To
