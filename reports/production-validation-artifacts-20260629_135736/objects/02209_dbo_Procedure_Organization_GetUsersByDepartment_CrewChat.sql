-- ─── PROCEDURE→FUNCTION: organization_getusersbydepartment_crewchat ───────────────────────────────
-- NOTE: SQL Server stored procedure converted to PostgreSQL function.
-- TODO: Review converted output — stored procedure semantics differ; test before use in production.
-- TODO: replace SETOF record — procedure returns results; add RETURNS TABLE(col type, ...) manually
-- TODO: procedure contains result-returning SELECT; replace SETOF record with correct column types
DROP FUNCTION IF EXISTS public.organization_getusersbydepartment_crewchat(integer, boolean);
CREATE OR REPLACE FUNCTION public.organization_getusersbydepartment_crewchat(
    IN departno integer,
...
BEGIN
	...
	INSERT INTO Sorting
	RETURN QUERY
	SELECT UserNo FROM Organization_SortingEachDepartment WHERE DepartNo = organization_getusersbydepartment_crewchat.departno
	...
	IF AlsoDisabled = 0 THEN
		...
		WHERE B.DepartNo = organization_getusersbydepartment_crewchat.departno
	...
	IF (SELECT COUNT(*) FROM Sorting) = 0 THEN
		...
		RETURN QUERY
		SELECT UserNo, ModUserNo, ModDate, UserID, Name, Name_EN, MailAddress, CellPhone, CompanyPhone, UserPhoto, Photo,
	...
	ELSE BEGIN
		...
		SELECT S.RowIdx, L.UserNo, L.ModUserNo, L.ModDate, L.UserID, L.Name, L.Name_EN, L.MailAddress, L.CellPhone, L.CompanyPhone, L.UserPhoto, L.Photo,
			...
			L.PositionName_JP,L.PositionName_VN,L.DutyName_CH,L.DutyName_JP	,L.DutyName_VN,L.BirthDate,L.EntranceDate,L.StateMessage
		...
		SELECT UserNo, ModUserNo, ModDate, UserID, Name, Name_EN, MailAddress, CellPhone, CompanyPhone, UserPhoto, Photo,
			...
			DutyNo, DutyName, DutyName_EN, DutySortNo
			,Name_CH,Name_JP	,Name_VN,DepartName_CH,DepartName_JP,DepartName_VN,PositionName_CH,PositionName_JP,PositionName_VN,DutyName_CH,
			DutyName_JP	,DutyName_VN,BirthDate,EntranceDate,StateMessage
		FROM ListOfUsers2 C
