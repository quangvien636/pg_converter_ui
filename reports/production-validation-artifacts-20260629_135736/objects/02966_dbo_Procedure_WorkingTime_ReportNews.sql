-- ─── PROCEDURE→FUNCTION: workingtime_reportnews ───────────────────────────────
...
BEGIN
			...
			CREATE TEMP TABLE ListOfUsers AS SELECT S.SortNo, P.SortNo PSortNo, DT.SortNo DTSortNo, D.SortNo AS DSortNo
				...
				,(cast(SUBSTRING(ST3.SettingValue,1,3) as int)*60+cast(SUBSTRING(ST3.SettingValue,5,2) as int))/ 60 TimeOffset FROM   (CREATE TEMP TABLE WKTIME AS SELECT u1.* FROM Organization_Users U1 
			...
			SELECT T.UserNo, T.TimeType, T.CheckTimeFull, T.CheckTimeC , T.StarWorking, Provider , T.WorkingDayC FROM WorkingTime_Times T
			WHERE t.WorkingDayC BETWEEN p_From AND p_To and COALESCE(t.status,0) != 1

			CREATE TEMP TABLE WKEdit AS SELECT e.UserNo, e.WorkingDayC FROM WKTIME e
			WHERE e.Provider = 999 
			group by e.UserNo, e.WorkingDayC

	---------------------------- End Filter user hide when no permission-------------------------

		IF NOW() <d_to SET p_To = CONVERT(VARCHAR, NOW(), 112) THEN
