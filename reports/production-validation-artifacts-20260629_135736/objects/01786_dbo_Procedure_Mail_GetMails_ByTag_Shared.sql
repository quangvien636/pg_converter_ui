-- ─── PROCEDURE→FUNCTION: mail_getmails_bytag_shared ───────────────────────────────
...
DROP FUNCTION IF EXISTS public.mail_getmails_bytag_shared(integer, integer, character varying, bigint, bigint, bigint, integer, boolean, integer, integer);
CREATE OR REPLACE FUNCTION public.mail_getmails_bytag_shared(
    IN userno integer,
...
BEGIN
	...
	EndRowNum := mail_getmails_bytag_shared.currentpageindex * CountPerPage;
	RETURN QUERY
	...
	LEFT JOIN (SELECT TagNo, ImageNo FROM Mail_MailTags WHERE UserNo = mail_getmails_bytag_shared.userno) MT ON MT.TagNo = M.TagNo
