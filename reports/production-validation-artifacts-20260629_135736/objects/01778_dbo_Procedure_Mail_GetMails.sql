-- ─── PROCEDURE→FUNCTION: mail_getmails ───────────────────────────────
...
DROP FUNCTION IF EXISTS public.mail_getmails(integer, integer, character varying, bigint, bigint, bigint, integer, bigint, integer, boolean, integer, integer, boolean);
CREATE OR REPLACE FUNCTION public.mail_getmails(
    IN userno integer,
...
BEGIN
	...
	IF SearchText != '' THEN
		...
		ELSIF (SearchType = 10) SET Query += 'AND Content ILIKE ''%'' || SearchText || ''%'' ESCAPE ''['' ' THEN
		...
		ELSIF SearchType = 2 THEN
			...
			(
				...
				+ ' OR To ILIKE ''%user_' || Convert(varchar,UserNo) + ',%''' 
				+ ' OR To = ''user_' || Convert(varchar,UserNo) + '''' 
				+ ' OR Cc ILIKE ''%,user_' || Convert(varchar,UserNo) + '''' 
				+ ' OR Cc ILIKE ''%user_' || Convert(varchar,UserNo) + ',%''' 
				+ ' OR Cc = ''user_' || Convert(varchar,UserNo) + '''' 
				FROM Organization_Users t where Name ILIKE '%' || SearchText || '%' FOR XML PATH('')
			), 1, 4, '')

			subWhere2 := STUFF(;
			(
				SELECT ' OR To ILIKE ''%,depart_0_' || Convert(varchar,DepartNo) + '''' 
				+ ' OR To ILIKE ''%depart_0_' || Convert(varchar,DepartNo) + ',%''' 
				+ ' OR To = ''depart_0_' || Convert(varchar,DepartNo) + '''' 
				+ ' OR Cc ILIKE ''%,depart_0_' || Convert(varchar,DepartNo) + '''' 
				+ ' OR Cc ILIKE ''%depart_0_' || Convert(varchar,DepartNo) + ',%''' 
				+ ' OR Cc = ''depart_0_' || Convert(varchar,DepartNo) + '''' 
				+ ' OR To ILIKE ''%,depart_1_' || Convert(varchar,DepartNo) + '''' 
				+ ' OR To ILIKE ''%depart_1_' || Convert(varchar,DepartNo) + ',%''' 
				+ ' OR To = ''depart_1_' || Convert(varchar,DepartNo) + '''' 
				+ ' OR Cc ILIKE ''%,depart_1_' || Convert(varchar,DepartNo) + '''' 
				+ ' OR Cc ILIKE ''%depart_1_' || Convert(varchar,DepartNo) + ',%''' 
				+ ' OR Cc = ''depart_1_' || Convert(varchar,DepartNo) + '''' 
				FROM Organization_Departments t where Name ILIKE '%' || SearchText || '%' FOR XML PATH('')
			), 1, 4, '')

			--print(subWhere)

			IF(subWhere2 != '' and subWhere2 is not null)
			BEGIN
				SET subWhere += 'OR' || subWhere2
			END;
		...
		ELSIF SearchType = 7 THEN
			...
			subWhere2 := STUFF(;
			(
				SELECT ' OR Cc ILIKE ''%,depart_0_' || Convert(varchar,DepartNo) + '''' 
				+ ' OR Cc ILIKE ''%depart_0_' || Convert(varchar,DepartNo) + ',%''' 
				+ ' OR Cc = ''depart_0_' || Convert(varchar,DepartNo) + '''' 
				+ ' OR Cc ILIKE ''%,depart_1_' || Convert(varchar,DepartNo) + '''' 
				+ ' OR Cc ILIKE ''%depart_1_' || Convert(varchar,DepartNo) + ',%''' 
		...
		ELSIF SearchType = 8 THEN
			...
			subWhere2 := STUFF(;
			(
				SELECT ' OR Bcc ILIKE ''%,depart_0_' || Convert(varchar,DepartNo) + '''' 
				+ ' OR Bcc ILIKE ''%depart_0_' || Convert(varchar,DepartNo) + ',%''' 
				+ ' OR Bcc = ''depart_0_' || Convert(varchar,DepartNo) + '''' 
				+ ' OR Bcc ILIKE ''%,depart_1_' || Convert(varchar,DepartNo) + '''' 
				+ ' OR Bcc ILIKE ''%depart_1_' || Convert(varchar,DepartNo) + ',%''' 
				+ ' OR Bcc = ''depart_1_' || Convert(varchar,DepartNo) + '''' 
				FROM Organization_Departments t where Name ILIKE '%' || SearchText || '%' FOR XML PATH('')
			), 1, 4, '')
	...
	EndRowNum := mail_getmails.currentpageindex * CountPerPage;
	IF ParentNo = -1 AND SortNo = 1 THEN
		...
		LEFT JOIN (SELECT TagNo, ImageNo FROM Mail_MailTags WHERE UserNo = mail_getmails.userno) MT ON MT.TagNo = M.TagNo
	...
	IF IsConversation = TRUE THEN
		...
		WHERE UserNo = mail_getmails.userno AND IsDelete = FALSE AND ConversationNo IN (SELECT T3.ConversationNo FROM SearchResult3 T3)
