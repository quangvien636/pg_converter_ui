-- ─── PROCEDURE→FUNCTION: mail_updatemail_formailbox_readdate ───────────────────────────────
...
BEGIN
	...
	CountOfUnReadMails := (SELECT UnReadCount FROM Mail_MailBoxs WHERE BoxNo = mail_updatemail_formailbox_readdate.boxno);;
	...
	UnReadCount := (SELECT UnReadCount FROM Mail_MailBoxs WHERE BoxNo = mail_updatemail_formailbox_readdate.boxno);
	IF ReadDate IS NOT NULL THEN
		...
		IF (SELECT COUNT(*) FROM AllBoxs WHERE BoxNo = mail_updatemail_formailbox_readdate.boxno) > 0 THEN
	...
	ELSE BEGIN
		...
		MailCount := (SELECT COUNT(*) FROM Mail_Mails WHERE BoxNo = mail_updatemail_formailbox_readdate.boxno AND IsDelete = FALSE);
		-- 태그별 메일 개수를 조정합니다.;
		UPDATE Mail_MailTags
		UnReadCount := UnReadCount - (SELECT UnReadCount FROM ListOfTags WHERE TagNo = Mail_MailTags.TagNo) + MailCount;
		WHERE TagNo IN (SELECT TagNo FROM ListOfTags)
