-- ─── PROCEDURE→FUNCTION: workingtime_employeenoouts ───────────────────────────────
-- NOTE: SQL Server stored procedure converted to PostgreSQL function.
-- TODO: Review converted output — stored procedure semantics differ; test before use in production.
-- TODO: replace SETOF record — procedure returns results; add RETURNS TABLE(col type, ...) manually
-- TODO: procedure contains result-returning SELECT; replace SETOF record with correct column types
DROP FUNCTION IF EXISTS public.workingtime_employeenoouts(integer, integer, integer, integer, character varying, character varying, character varying);
CREATE OR REPLACE FUNCTION public.workingtime_employeenoouts(
    IN p_from integer,
    IN p_to integer,
...
BEGIN
	...
	where (t.TimeType = 3 or t.TimeType < 0) and COALESCE(WorkingDayC, WT2.WorkingDayOfCompany) BETWEEN p_From AND p_To
	RETURN QUERY
	SELECT 
	...
	LEFT JOIN Organization_SortingEachDepartment S ON S.UserNo = U.UserNo and S.DepartNo = workingtime_employeenoouts.p_departno
	...
	AND ( (p_GroupNo = 0 OR U.GroupId = workingtime_employeenoouts.p_groupno) OR (p_GroupNo !=  0 AND U.GroupId = workingtime_employeenoouts.p_groupno))
	AND ( (p_GroupNo = 0 OR G.ID = workingtime_employeenoouts.p_groupno) OR (p_GroupNo !=  0 AND G.ID = workingtime_employeenoouts.p_groupno))
