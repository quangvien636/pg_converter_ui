-- ─── PROCEDURE→FUNCTION: schedule_insertresourcenotificationservice ───────────────────────────────
-- NOTE: SQL Server stored procedure converted to PostgreSQL function.
-- TODO: Review converted output — stored procedure semantics differ; test before use in production.
-- TODO: TOP was preserved as comment; add LIMIT manually
DROP FUNCTION IF EXISTS public.schedule_insertresourcenotificationservice(integer, character varying, integer, integer, character varying, character varying, date, date, character varying, character varying, boolean, xml, date);
CREATE OR REPLACE FUNCTION public.schedule_insertresourcenotificationservice(
...
BEGIN
	...
	-- get Notification ID
	CREATE TEMP TABLE tb AS SELECT /* TOP 1 */ NotificationNo = NotificationNo from  Center_NotificationService where ProjectCode = schedule_insertresourcenotificationservice.projectcode and Connectionkey = schedule_insertresourcenotificationservice.connectionkey  
