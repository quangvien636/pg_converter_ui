-- ─── PROCEDURE→FUNCTION: organization_getalluserswithbelongs_crewchat ───────────────────────────────
-- NOTE: SQL Server stored procedure converted to PostgreSQL function.
-- TODO: Review converted output — stored procedure semantics differ; test before use in production.
-- TODO: replace SETOF record — procedure returns results; add RETURNS TABLE(col type, ...) manually
-- TODO: procedure contains result-returning SELECT; replace SETOF record with correct column types
DROP FUNCTION IF EXISTS public.organization_getalluserswithbelongs_crewchat(boolean);
CREATE OR REPLACE FUNCTION public.organization_getalluserswithbelongs_crewchat(
...
BEGIN
	...
	IF AlsoDisabled = 0 THEN
		...
		SELECT B.UserNo, U.ModUserNo, U.ModDate, U.UserID, U.Name, U.Name_EN
			...
			, U.MailAddress, U.CellPhone, U.CompanyPhone, U.ExtensionNumber,U.UserPhoto, U.Photo, U.Enabled,
			P.SortNo AS PositionSortNo, U.BirthDate, U.EntranceDate,COALESCE(C.StateMessage,'') as StateMessage
	...
	IF (SELECT COUNT(*) FROM Sorting) = 0 THEN
		...
		, MailAddress, CellPhone, CompanyPhone,ExtensionNumber, UserPhoto, Photo, Enabled,
			PositionSortNo, BirthDate, EntranceDate,StateMessage
		FROM ListOfUsers
	...
	ELSE BEGIN
		...
		, L.MailAddress, L.CellPhone, L.CompanyPhone,L.ExtensionNumber, L.UserPhoto, L.Photo, L.Enabled,
			L.PositionSortNo, L.BirthDate, L.EntranceDate,L.StateMessage
		...
		, Name_CH, Name_JP, Name_VN
		, MailAddress, CellPhone, CompanyPhone, ExtensionNumber,UserPhoto,
			Photo, Enabled, PositionSortNo, BirthDate, EntranceDate,PositionSortNo,StateMessage
		FROM ListOfUsers2 C
	...
	RETURN QUERY
	SELECT DISTINCT 
		ROW_NUMBER() OVER( ORDER BY COALESCE(S.SortNo,0) , P.SortNo , U.Name ) AS UserSortNo,
