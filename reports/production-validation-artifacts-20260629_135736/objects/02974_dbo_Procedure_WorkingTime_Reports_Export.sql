-- ─── PROCEDURE→FUNCTION: workingtime_reports_export ───────────────────────────────
...
BEGIN
	...
	WHILE @FETCH_STATUS = 0 BEGIN
		...
		TotalLate := 0;
		TotalEdit := 0;
		SELECT sum(case when (wc.TimeType = -2 or wc.TimeType = -3) then 4 else 8 end ), Count(*) INTO totaltime2, totalvacation from WorkingTime_Times wc where wc.UserNo = _UserNo and  wc.TimeType < 0  and wc.WorkingDayC BETWEEN FromDate AND ToDate
