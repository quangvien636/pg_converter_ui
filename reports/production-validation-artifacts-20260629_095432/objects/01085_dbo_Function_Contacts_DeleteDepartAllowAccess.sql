-- ─── FUNCTION: contacts_deletedepartallowaccess ───────────────────────────────
...
BEGIN
	...
	SELECT * 
	INTO   #Contact_DepartAllowAccess
	FROM Contact_DepartAllowAccess
	WHERE AllowAccessNo IN(SELECT * FROM SplitString(ListAllowAccessNo,','))


	WHILE (Select Count(*) From #Contact_DepartAllowAccess) > 0
	BEGIN
			...
			SELECT * INTO   #ShareGroupTemp FROM ShareGroupNos
