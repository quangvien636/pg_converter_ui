-- ─── FUNCTION: edmsdocinsert2 ───────────────────────────────
DROP FUNCTION IF EXISTS public.edmsdocinsert2();
CREATE OR REPLACE FUNCTION public.edmsdocinsert2(
) RETURNS TABLE(
    id text
)
...
select	TITLE			= 'ㄴㅇㅎ' 
	...
	***************************************************************************/
	...
	select	valdate	=	 case when STOREPERIOD > 0 and STOREPERIOD <> '' then dateadd(month,convert(int,STOREPERIOD),NOW()-1)
	...
	***************************************************************************/
	...
	update	edmsdocument
	set		Serial = public."EDMSRETURNSerial"(EDMSDOCID,(SELECT VALUEDATA FROM EDMSConfiguration WHERE KEYCODE = 'DocSerial'),USERID,NOW()-1)
	,       WGROUP = EDMSDOCID
	where	id = EDMSDOCID
