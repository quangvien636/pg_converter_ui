-- ─── FUNCTION: notice_getdivisionscount ───────────────────────────────
DROP FUNCTION IF EXISTS public.notice_getdivisionscount();
CREATE OR REPLACE FUNCTION public.notice_getdivisionscount(
) RETURNS TABLE(
    departno text
)
AS $function$
...
BEGIN
	...
	SELECT UserNo = UserNo FROM Organization_Users WHERE UserID = UserId

	SELECT ADMIN = COUNT(x.UserNo) FROM(
		select m.UserNo 
	...
	SELECT
		N.DivisionNo,
		COUNT(N.NoticeNo) AS AllCnt,
		SUM(CASE WHEN  NOW() <= N.EndDate AND (A.RCnt IS NULL) AND COALESCE(N.IsSeen2, 0) = 0 THEN 1 ELSE 0 END) AS UnCnt --20231116
		,SUM(CASE WHEN  NOW() > N.EndDate AND (A.RCnt IS NULL) AND COALESCE(N.IsSeen2, 0) = 0 THEN 1 ELSE 0 END) AS EXD
	FROM 
	 Notices N
	...
	WHERE  ((IsShare = FALSE 
			...
			AND (ADMIN > 0 OR  NOW() >= N.STARTDATE)
