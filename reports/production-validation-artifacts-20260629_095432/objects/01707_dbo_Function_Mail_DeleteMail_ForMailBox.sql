-- ─── FUNCTION: mail_deletemail_formailbox ───────────────────────────────
...
BEGIN
	...
	IF ((SELECT COUNT(*) FROM AllBoxs WHERE BoxNo = mail_deletemail_formailbox.boxno) > 0) BEGIN


		SELECT TotalCount = TotalCount, UnReadCount = UnReadCount FROM Mail_MailBoxs WHERE BoxNo = mail_deletemail_formailbox.boxno
