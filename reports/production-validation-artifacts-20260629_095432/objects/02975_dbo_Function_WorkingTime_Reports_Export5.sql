-- ─── FUNCTION: workingtime_reports_export5 ───────────────────────────────
DROP FUNCTION IF EXISTS public.workingtime_reports_export5(character varying, character varying, integer, integer, character varying, character varying, character varying);
CREATE OR REPLACE FUNCTION public.workingtime_reports_export5(
...
---user dayoff
			...
			LEFT JOIN Organization_SortingEachDepartment S ON S.UserNo = U.UserNo and S.DepartNo = workingtime_reports_export5.p_departno
			...
			AND ( (p_GroupNo = 0 OR U.GroupId = workingtime_reports_export5.p_groupno) OR (p_GroupNo !=  0 AND U.GroupId = workingtime_reports_export5.p_groupno))
	...
	DEALLOCATE WorkingTime_Cursor
		SELECT UserNo
			   ...
			   , TotalNoCheckOut
			   , TotalEdit
		FROM ListOfReport where DayOff > 0;
END;
$function$
LANGUAGE plpgsql;
