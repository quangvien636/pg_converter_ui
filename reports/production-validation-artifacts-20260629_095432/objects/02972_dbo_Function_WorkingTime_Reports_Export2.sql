-- ─── FUNCTION: workingtime_reports_export2 ───────────────────────────────
DROP FUNCTION IF EXISTS public.workingtime_reports_export2(character varying, character varying, integer, integer, character varying, character varying, character varying);
CREATE OR REPLACE FUNCTION public.workingtime_reports_export2(
...
BEGIN
			...
			LEFT JOIN Organization_SortingEachDepartment S ON S.UserNo = U.UserNo and S.DepartNo = workingtime_reports_export2.p_departno
			...
			AND ( (p_GroupNo = 0 OR U.GroupId = workingtime_reports_export2.p_groupno) OR (p_GroupNo !=  0 AND U.GroupId = workingtime_reports_export2.p_groupno))
	...
	DEALLOCATE WorkingTime_Cursor
		SELECT UserNo
			   ...
			   , TotalNoCheckOut
			   , TotalEdit
		FROM ListOfReport where TotalNoCheckIn > 0;
END;
$function$
LANGUAGE plpgsql;
