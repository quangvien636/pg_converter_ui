-- ─── FUNCTION: workingtime_reports2 ───────────────────────────────
DROP FUNCTION IF EXISTS public.workingtime_reports2(integer, character varying, character varying, integer, integer, integer, integer, integer, character varying, character varying, character varying);
CREATE OR REPLACE FUNCTION public.workingtime_reports2(
    userno integer,
...
BEGIN
	...
	insert into ListOfUsers
			...
			LEFT JOIN Organization_SortingEachDepartment S ON S.UserNo = U.UserNo and S.DepartNo = workingtime_reports2.departno
			...
			AND ( (p_GroupNo = 0 OR U.GroupId = workingtime_reports2.p_groupno) OR (p_GroupNo !=  0 AND U.GroupId = workingtime_reports2.p_groupno))
	...
	WHILE @FETCH_STATUS = 0 BEGIN
		...
		SELECT TotalNoCheckIn = (TotalDays - COUNT(*))
		FROM (SELECT _UserNo AS UserNo FROM ReturnWorkingTime WHERE Status = 1 AND (TimeType = 1 OR TimeType = 8) GROUP BY WorkingDay) AS CheckIn

		SELECT TotalNoCheckOut = (TotalDays - COUNT(*))
		FROM (SELECT _UserNo AS UserNo FROM ReturnWorkingTime WHERE Status = 1 AND (TimeType = 3 OR TimeType = 6) GROUP BY WorkingDay) AS CheckOut
		...
		INSERT INTO ListOfReport (UserNo, UserID, Name, Name_EN, DepartName, DepartName_EN, PositionName, PositionName_EN, DutyName, DutyName_EN, TotalNoCheckIn, TotalNoCheckOut, TotalLate, TotalEdit, TimeWork, DayOff, idx)
		VALUES (_UserNo, UserID, Name, Name_EN, DepartName, DepartName_EN, PositionName, PositionName_EN, DutyName, DutyName_EN, TotalNoCheckIn, TotalNoCheckOut, TotalLate, TotalEdit, TotalTime, TotalDayOff, _idx) 
		set _idx = _idx+1
		FETCH NEXT FROM WorkingTime_Cursor
			INTO _UserNo, UserID, Name, Name_EN, DepartName, DepartName_EN, PositionName, PositionName_EN, DutyName, DutyName_EN,TimeIn, TimeOffset
		END
	...
	BEGIN
		RETURN QUERY
		SELECT UserNo, Name, Name_EN, UserID, DepartName, DepartName_EN, PositionName, PositionName_EN, DutyName, DutyName_EN, TotalNoCheckIn, TotalNoCheckOut, TotalLate, TotalEdit, COALESCE(TimeWork,0) TimeWork, COALESCE(DayOff,0) DayOff FROM ListOfReport
	END
	ELSE
	BEGIN
		RETURN QUERY
		SELECT UserNo, Name, Name_EN, UserID, DepartName, DepartName_EN, PositionName, PositionName_EN, DutyName, DutyName_EN, TotalNoCheckIn, TotalNoCheckOut, TotalLate, TotalEdit, COALESCE(TimeWork,0) TimeWork,COALESCE(DayOff,0) DayOff
		FROM ListOfReport L
		WHERE L.idx BETWEEN ((CurrentPageIndex - 1)*ViewCount+1) AND (CurrentPageIndex*ViewCount)
	END
