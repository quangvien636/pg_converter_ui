-- ─── FUNCTION: workingtime_reports_export6 ───────────────────────────────
DROP FUNCTION IF EXISTS public.workingtime_reports_export6(character varying, character varying, integer, integer, character varying, character varying, character varying);
CREATE OR REPLACE FUNCTION public.workingtime_reports_export6(
...
---user outside
			...
			LEFT JOIN Organization_SortingEachDepartment S ON S.UserNo = U.UserNo and S.DepartNo = workingtime_reports_export6.p_departno
			...
			AND ( (p_GroupNo = 0 OR U.GroupId = workingtime_reports_export6.p_groupno) OR (p_GroupNo !=  0 AND U.GroupId = workingtime_reports_export6.p_groupno))
			...
			AND B.UserNo IN(
					...
					where t.TimeType = 2 and COALESCE(WorkingDayC, WT2.WorkingDayOfCompany) BETWEEN p_From AND p_To
