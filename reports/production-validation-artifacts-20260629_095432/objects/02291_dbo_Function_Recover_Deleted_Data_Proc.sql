-- ─── FUNCTION: recover_deleted_data_proc ───────────────────────────────
...
CREATE OR REPLACE FUNCTION public.recover_deleted_data_proc(
    ...
    schemaname_n_tablename character varying,
    date_from timestamp without time zone DEFAULT '1900/01/01',
    date_to timestamp without time zone DEFAULT '9999/12/31'
) RETURNS TABLE(
    col1 text
)
...
IF COALESCE(Compatibility_Level,0)<=80
...
IF (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA || '.' || TABLE_NAME=recover_deleted_data_proc.schemaname_n_tablename)=0
...
(
  ...
  Update INT
        ...
        BEGIN
            ...
            SET Fileid=SUBSTRING(ConsolidatedPageID,0,STRPOS(ConsolidatedPageID, ':')) -- Seperate File ID from Page ID

         

            SET hex_pageid ='0x' || SUBSTRING(ConsolidatedPageID,STRPOS(ConsolidatedPageID, ':')+1,Len(ConsolidatedPageID))  ---Seperate the page ID
