...
--*/;
	...
	if exists(select valuedata from EDMSConfiguration where KeyCode='TranEADocSerial' and ValueData='1')
	  begin 
		...
		UPDATE	EDMSDocument	
		SET		DocState = '0'				--사용안함으로 등록된 내역을 사용함으로 수정.
	  ...
	  begin
		...
		if (select docstate from EDMSDocument where ID=EDDOCUMENTID) = '1'
		begin
			exec EDMSSerialSetting EDDOCUMENTID
		  
			UPDATE	EDMSDocument	
			SET		Serial = public."EDMSRETURNSerial"(EDDOCUMENTID,(SELECT VALUEDATA FROM EDMSConfiguration WHERE KEYCODE = 'DocSerial'),WriterID,RegDate) --시리얼번호 리턴
			WHERE	ID = EDDOCUMENTID	
		end	
	...
	UPDATE	EAPPDocument	
	SET		EDMSState = '200'			--이관 성공 상태로 변경
	...
	***************************************************************************/           	
	...
	begin
		...
		select SaupName = SaupjangName,SaupCode = SaupjangCode from EAPPDocUserSerial where DocId = edmstran.eadocid
