-- ─── FUNCTION: workingtime_reports_export7 ───────────────────────────────
DROP FUNCTION IF EXISTS public.workingtime_reports_export7(character varying, character varying, integer, integer, character varying, character varying, character varying);
CREATE OR REPLACE FUNCTION public.workingtime_reports_export7(
...
---user edit
			...
			LEFT JOIN Organization_SortingEachDepartment S ON S.UserNo = U.UserNo and S.DepartNo = workingtime_reports_export7.p_departno
			...
			AND ( (p_GroupNo = 0 OR U.GroupId = workingtime_reports_export7.p_groupno) OR (p_GroupNo !=  0 AND U.GroupId = workingtime_reports_export7.p_groupno))
			...
			AND B.UserNo IN(
					...
					where t.Provider = 999 and COALESCE(WorkingDayC, WT2.WorkingDayOfCompany) BETWEEN p_From AND p_To
			)

	---------------------------- End Filter user hide when no permission-------------------------

	IF NOW() < CONVERT(DATETIME, p_To, 1) SET p_To = CONVERT(VARCHAR, NOW(), 112)
	...
	WHILE @FETCH_STATUS = 0 BEGIN
		...
		SET TotalLate = 0
		SET TotalEdit = 0
		select TotalTime2 =  sum(case when (wc.TimeType = -2 or wc.TimeType = -3) then 4 else 8 end ), TotalVacation =  Count(*)  from WorkingTime_Times wc where wc.UserNo = _UserNo and  wc.TimeType < 0  and wc.WorkingDayC BETWEEN p_From AND p_To
		select TotalTime =  sum(c.TimeWork)  from WorkingTime_Calculater c where c.UserNo = _UserNo and  c.status = 1 and c.WorkingDay BETWEEN p_From AND p_To
		select TotalDayOff =  (TotalDays  - COUNT(*))  from WorkingTime_Calculater c where c.UserNo = _UserNo  and c.WorkingDay BETWEEN p_From AND p_To
