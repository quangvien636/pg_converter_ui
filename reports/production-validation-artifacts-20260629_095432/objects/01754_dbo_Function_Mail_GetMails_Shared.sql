...
BEGIN
	...
	ELSE IF (SortColumn = 9) SET Query += 'COALESCE((select /* TOP 1 */ CommentsType from Mail_SharedComments where MailNo = Mail_Mails.MailNo  order by ModDate desc),0) '
	ELSE IF (SortColumn = 10) SET Query += '(select count(*) from Mail_SharedReference where MailNo = Mail_Mails.MailNo) '
...
*/
	...
	SET EndRowNum = mail_getmails_shared.currentpageindex * CountPerPage
