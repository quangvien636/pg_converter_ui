...
SELECT * FROM EDMSDOCUMENT
	...
	***************************************************************************/
	...
	select	valdate	=	 case when STOREPERIOD > 0  and STOREPERIOD <> '' then dateadd(month,convert(int,STOREPERIOD),NOW())
							else dateadd(year,1000,NOW())
								end


	SELECT EADOCID =  MAX(EADOCFLAG) FROM EDMSDOCUMENT WHERE ID = WGROUPCODE --재설정
	...
	BEGIN			
			...
			***************************************************************************/			
			...
			-- 시리얼 인서트.
			***************************************************************************/
			if (select COALESCE(valuedata,'') from EDMSConfiguration where KeyCode='VersionUpSerialKeep') = '1'
			  begin ;
				update	edmsdocument
				set		Serial = (select Serial from EDMSDocument where ID=WGROUPCODE)
				where	id = EDMSDOCID 
			  end
			else
			  begin
				...
				update	edmsdocument
				set		Serial = public."EDMSRETURNSerial"(EDMSDOCID,(SELECT VALUEDATA FROM EDMSConfiguration WHERE KEYCODE = 'DocSerial'),WriterID,RegDate)
				where	id = EDMSDOCID 
			  end
