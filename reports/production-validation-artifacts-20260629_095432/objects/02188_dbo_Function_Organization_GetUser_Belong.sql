-- ─── FUNCTION: organization_getuser_belong ───────────────────────────────
DROP FUNCTION IF EXISTS public.organization_getuser_belong(integer);
CREATE OR REPLACE FUNCTION public.organization_getuser_belong(
    userno integer
) RETURNS TABLE(
    belongno text,
    departno text,
...
BEGIN
	...
	IF (UserNo != 0) BEGIN
		...
		WHERE UserNo = organization_getuser_belong.userno
	...
	ELSE BEGIN
		...
		SELECT _UserNo = organization_getuser_belong.userno
		FROM Organization_Users
		WHERE UserID = UserID
