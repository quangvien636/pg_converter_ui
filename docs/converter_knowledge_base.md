...
| Single-table aggregate ambiguity | Aggregate reads one table; WHERE column matches a qualified function parameter | Add a table alias and qualify only the confirmed column | Four Board count/max procedures | Low when procedure-scoped | Verify unchanged aggregate/result shape and runtime execution | Exact table, alias, and column per procedure | Already implemented |
| Single-table getter ambiguity | Getter reads one known table; filter/projection column matches an input parameter | Add a procedure-specific alias; qualify confirmed filter and projection columns | Sixteen Contact getters | Low when procedure-scoped | Verify column order/names remain unchanged and execute under rollback | Exact table mapping plus projection collision case | Already implemented |
...
| TINYINT in body DDL | `CREATE TABLE #t (... TINYINT ...)` | `smallint` | `contacts_saveaddressinfo_web`, `contacts_updateuserinfo` | Low | Confirm the temporary-table statement executes | Temp table and normal table body DDL | Already implemented |
