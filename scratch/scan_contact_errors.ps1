$sqlFiles = Get-ChildItem -Path reports/contact_failing_sql -Filter *.sql
$failures = @()
...
foreach ($file in $sqlFiles) {
    ...
    if ($errors.Count -gt 0) {
        $failures += [PSCustomObject]@{
            procedure = $procName
            source_file = "reports/contact_failing_sql/$procName.sql"
            errors = $errors
        }
    }
}
...
$output | ConvertTo-Json -Depth 10 | Out-File -FilePath reports/fix-contact-tasks.json -Encoding utf8
Write-Output "Successfully wrote $($failures.Count) procedure failures to reports/fix-contact-tasks.json"
